...
If you lose the Administrator password and you no longer have the private key, you must reset the password or create a new instance\. For more information, see [Resetting a Lost or Expired Windows Administrator Password](ResettingAdminPassword.md) For steps to reset the password using an AWS Systems Manager document, see [Reset Passwords and SSH Keys on Amazon EC2 Instances](url-doc-domain;systems-manager/latest/userguide/automation-ec2reset.html)\. To change the password with AWS\-RunPowerShellScript run commmand, see the steps under **Changing the Administrator Password After Connecting** on the [Setting Passwords for Windows Instances](url-doc-domain;AWSEC2/latest/WindowsGuide/ec2-windows-passwords.html)page\. 