...
|  Change the **instance size** within the same instance family  |  Linux only  |  The reservation must use default tenancy\. Some instance families are not supported, because there are no other sizes available\. For more information, see [Support for Modifying Instance Sizes](https://docs.aws.amazon.com/AWSEC2/latest/WindowsGuide/ri-modifying.html#ri-modification-instancemove) in the *Amazon EC2 User Guide for Linux Instances*\.  | 
