...
+ Executes user data \(if specified\)\. For more information about specifying user data, see [Configuring Instances with User Data](ec2-instance-metadata.md#instancedata-add-user-data)\.
...
+ [Initialize Drives and Drive Letter Mappings](#ec2launch-mapping)
...
### Initialize Drives and Drive Letter Mappings<a name="ec2launch-mapping"></a>
...
1. In Windows PowerShell, run the following command so that the system schedules the script to run as a Windows Scheduled Task\.
