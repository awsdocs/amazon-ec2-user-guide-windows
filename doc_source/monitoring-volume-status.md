...
|  Avg Read Size \(KiB/Operation\)  |  `Avg(VolumeReadBytes) / 1024`  For C5 and M5 instances, use the following formula to derive Average Read Size: `(Sum(VolumeReadBytes) / Sum(VolumeReadOps)) / 1024` The `VolumeReadBytes` and `VolumeReadOps` metrics are available in the EBS CloudWatch console\.   | 
|  Avg Write Size \(KiB/Operation\)  |  `Avg(VolumeWriteBytes) / 1024`  For C5 and M5 instances, use the following formula to derive Average Write Size: `(Sum(VolumeWriteBytes) / Sum(VolumeWriteOps)) / 1024` The `VolumeWriteBytes` and `VolumeWriteOps` metrics are available in the EBS CloudWatch console\.   | 
|  Avg Read Latency \(ms/Operation\)  |  `Avg(VolumeTotalReadTime) × 1000`  For C5 and M5 instances, use the following formula to derive Average Read Latency: `(Sum(VolumeTotalReadTime) / Sum(VolumeReadOps)) × 1000` The `VolumeTotalReadTime` and `VolumeReadOps` metrics are available in the EBS CloudWatch console\.   | 
|  Avg Write Latency \(ms/Operation\)  |  `Avg(VolumeTotalWriteTime) × 1000`  For C5 and M5 instances, use the following formula to derive Average Write Latency: `(Sum(VolumeTotalWriteTime) / Sum(VolumeWriteOps)) x 1000` The `VolumeTotalWriteTime` and `VolumeWriteOps` metrics are available in the EBS CloudWatch console\.   | 
