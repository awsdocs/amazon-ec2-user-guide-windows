...
1. For **Launch template contents**, provide the following information:
   ...
   + **Security Groups**: One or more security groups to associate with the instance\. For more information, see [Amazon EC2 Security Groups for Windows Instances](using-network-security.md)\.
...
1. For **Storage \(Volumes\)**, specify volumes to attach to the instance besides the volumes specified by the AMI\.
   ...
   + **Encrypted**: Allows you to change the encryption state of a volume created by the template\.
     ...
     You apply encryption to EBS volumes by setting the `Encrypted` parameter to `true`\. \(The `Encrypted` parameter is optional if [ encryption by default](https://docs.aws.amazon.com/AWSEC2/latest/WindowsGuide/EBSEncryption.html#encryption-by-default) is enabled\)\.
...
**Encryption Outcomes**    
[\[See the AWS documentation website for more details\]](http://docs.aws.amazon.com/AWSEC2/latest/WindowsGuide/ec2-launch-templates.html)
