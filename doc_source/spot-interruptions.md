...
+ **Spot Instance request type** – must be `persistent`\. You can't specify a launch group in the Spot Instance request\.
+ **EC2 Fleet or Spot Fleet request type** – must be `maintain`
+ **Supported Windows operating systems** \(You must install the hibernation agent on a supported operating system\. Alternatively, use a supported AMI, which already includes the agent\.\):
  + Windows Server 2008 R2 and later
+ **Supported Windows AMIs** \(the following supported AMIs include the hibernation agent\):
  ...
  For information about the supported Linux AMIs, see [Supported Linux AMIs](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/spot-interruptions.html#hibernate-spot-instances) in the *Amazon EC2 User Guide for Linux Instances*\.
+ **Supported instance families** – C3, C4, C5, M4, M5, R3, R4
+ **Instance RAM size** –  can be up to 16 GB
